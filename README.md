...
### 초기 고민
os를 공부하다가 배운 CPU 스케줄링 알고리즘이 실제로 어떻게 다른지, 왜 Linux가 CFS를 선택했는지 직접 측정하고 싶었습니다.

### 핵심 인사이트: 목표 기반 테스트
단순히 "모든 테스트에서 3개 스케줄러 비교"가 아니라, **각 테스트 목표에 맞는 스케줄러만 선택적으로 비교**하기로 결정했습니다.

- **일반 워크로드**: 3개 모두 비교 (알고리즘 효율성)
- **공정성 테스트**: MLFQS vs CFS만 (Basic은 starvation 위험)
- **Nice 효과**: MLFQS vs CFS만 (Basic의 nice는 정적 우선순위로 다른 의미)

이 결정이 프로젝트의 가장 중요한 차별점이 되었습니다.

### 스케줄러 선택
1. **Basic Priority**: Baseline, 가장 단순
2. **MLFQS (64-Queue)**: FreeBSD 방식, 동적 피드백
3. **CFS**: Linux 커널, vruntime 기반 공정성

---

## 3. 주요 트러블슈팅

### Bug #1: Time Slice 미구현
**증상:** 모든 스케줄러가 정확히 같은 결과 (8215.2 ticks)

**원인:** 스레드가 완료될 때까지 계속 실행, 선점 없음

**해결:** `time_slice = 4 ticks` 추가, 만료 시 thread_yield() 호출

**배운 점:** 이론에서 당연한 것도 구현에서는 명시적으로 추가해야 함

### Bug #2: CFS vruntime 정밀도 문제
**증상:** vruntime이 항상 0

**원인:** 정수 나눗셈에서 정밀도 손실
```python
# 잘못된 코드
(1 * 1024) // 88761 = 0  # nice -20일 때

# 수정된 코드 (1000배 스케일)
(1 * 1024 * 1000) // 88761 = 11
```

**결과:** nice -20 vs 19에서 6206배 차이 (이론값 5917배의 105%)

**배운 점:** 정수 연산에서는 충분히 큰 스케일 필요, Linux 커널도 동일한 기법 사용

### Bug #3: MLFQS Nice 덮어쓰기
**증상:** Nice 효과 테스트에서 CPU time ratio가 1:1

**원인:** `add_thread()`에서 `thread.nice = 0`으로 모든 nice 값 초기화

**해결:** 해당 라인 제거, 워크로드의 nice 값 보존

**결과:** CPU time ratio가 1:1 → 62499:1로 정상화

**배운 점:** 초기화할 값(recent_cpu)과 보존할 값(nice)의 구분

### Bug #4: Nice 효과 측정 방법론
**증상:** Nice 버그 수정 후에도 여전히 1:1

**원인:** 모든 스레드가 완료되면 각자 burst_time만큼 사용 → nice와 무관하게 동일

**해결:**
- burst_time: 300 → 10,000으로 증가
- 시뮬레이션을 50% 시간에 중단

**결과:** MLFQS 62499:1, CFS 2499:1

**배운 점:** "무엇을" 측정하는가만큼 "어떻게" 측정하는가가 중요

### Bug #5: Basic Priority FIFO 위반
**증상:** Basic이 거의 항상 승리 (80%+)

**원인:** `max(key=lambda t: (t.priority, -t.tid))`가 TID로 정렬 (FIFO 아님)

**해결:** 진짜 FIFO 구현 (ready_queue 순회하며 첫 번째 최고 우선순위 선택)

**결과:** 승률 80% → 38%로 정상화

### Bug #6: 컨텍스트 스위치 카운트 누락
**증상:** context_switches가 항상 0

**해결:** `prev_running_tid`로 이전 스레드 추적, 스위치 발생 시 카운트
...
### Burst Time은 정적
**질문:** "burst_time을 vruntime에 맞게 동적으로 늘려야 하나?"

**답변:** NO. burst_time은 "실제 작업량"이고, 스케줄러는 "배분 방법"만 결정. 작업량 자체는 변하지 않음.

### 시뮬레이션 파라미터
- **Time Slice**: 4 ticks (리눅스 4ms와 유사)
- **Max Ticks**: 35,000 (50개 스레드 완료 + 여유)
- **Thread Count**: 50 (기본), 확장성 테스트는 10/100/500
...
### 이론과 구현의 괴리
- Time slice: 이론에서는 암묵적, 구현에서는 명시적
- FIFO: TID가 아닌 큐 삽입 순서
- 정밀도: 수학적 나눗셈 ≠ 정수 나눗셈

### 정수 연산과 고정소수점
운영체제는 부동소수점을 피하고 스케일로 정밀도 확보:
- MLFQS: 17.14 고정소수점
- CFS: 1024 × 1000 스케일
...
| 목표 | 잘못된 방법 | 올바른 방법 |
|------|------------|------------|
| Nice 효과 | 모두 완료 후 측정 | 50% 시간에 중단 |
| 공정성 | 모든 스케줄러 비교 | 공정성 목표만 |

### 디버깅 전략
이상한 결과는 버그의 징후:
- 모든 결과 같음 → Time slice 미구현
- 한 쪽 압도적 → 구현 버그
- 메트릭 항상 0 → 측정 오류
...
### 정량적
- **코드**: 2,939줄, 21개 파일
- **구현**: 3개 스케줄러, 8가지 워크로드, 13개 테스트
- **버그 수정**: 6개 major bugs
- **배포**: GCP VM, systemd 서비스로 24/7 운영
- **최종 결과**: Basic 38%, MLFQS 38%, CFS 23% (편향 없음)

### 정성적
1. 스케줄러를 "구현 가능한 알고리즘"으로 이해
2. 트레이드오프 체험 (단순함 vs 공정성 vs 응답성)
3. FreeBSD MLFQS, Linux CFS 실제 구현 참조
4. 정량적 분석 능력 획득

### 예상치 못한 발견
- Nice 효과: MLFQS 62499:1 vs CFS 2499:1 (극단적 차이)
- Basic Priority의 건투: 단순하지만 특정 워크로드에서 승리
- I/O bound 우대: MLFQS가 명확히 유리
...
### 구현 못한 기능
- **멀티 CPU**: 복잡도 급증 (load balancing, cache coherency)
- **Real-time 스케줄러**: SCHED_FIFO, SCHED_RR
- **I/O 스케줄러 통합**: CFQ, Deadline 등

### 측정의 한계
- CFS 이론값(5917:1)과 실제값(2499:1) 괴리 (42%)
- Starvation 측정: 플래그만, 실제 분포 미측정
...
### 기술적 성장
**Before:** CPU 스케줄링은 시험용 이론
**After:** 구현 가능하고 트레이드오프를 측정할 수 있는 알고리즘

### 프로젝트 문서화
- README 작성으로 설계 결정 이유 명확화
- 단일 커밋으로 완성 → 개별 커밋으로 했다면 더 명확한 히스토리

### 핵심 인사이트
**운영체제는 Trade-off의 예술**
- Basic: 단순, 예측 가능, 하지만 불공정
- MLFQS: 동적, 응답성, 하지만 복잡
- CFS: 완벽한 공정성, 하지만 I/O 우대 없음

**완벽한 스케줄러는 없습니다.** 워크로드와 시스템에 맞는 선택이 필요합니다.

---

## 9. 결론

이 프로젝트는 단순히 "스케줄러 3개 구현"이 아니라, **운영체제 핵심 트레이드오프 체험**이었습니다. 6가지 버그를 수정하며 이론과 구현의 괴리를 이해했고, 목표 기반 테스트로 공정한 비교 방법론을 확립했습니다.

### 최종 성과
- 2,939줄, 3개 스케줄러, 13개 테스트
- GCP VM 24/7 운영 (http://34.47.105.226)
- 편향 없는 결과 (각 스케줄러가 자신의 강점에서 승리)

### 감사
- PintOS 문서 (MLFQS 고정소수점)
- Linux 커널 소스 (CFS 가중치 테이블)
- FreeBSD 소스 (MLFQS 구현)
- StackOverflow (정수 나눗셈 해결)
...
- **배포**: GCP VM에 24/7 운영 중 (http://34.47.105.226)
