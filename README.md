...
- [주요 발견사항](#주요-발견사항)
- [설계 결정사항](#설계-결정사항)
- [스케줄러 벤치마크 프로젝트 소감문](#스케줄러-벤치마크-프로젝트-소감문)
...
- **cpu_bound**: Nice 0 (순수 알고리즘 효율성 비교)
...
### 4. 일관성 (4개 테스트, 3-way 비교)

**목적**: 대기 시간의 예측 가능성과 일관성 측정 (CFS 유리)

| 테스트 | 워크로드 | 주요 메트릭 | 비교 대상 |
|--------|---------|------------|-----------|
| 일관성: 변동계수 | mixed | cv_wait | Basic + MLFQS + CFS |
| 일관성: P99 레이턴시 | mixed | p99_wait | Basic + MLFQS + CFS |
| 일관성: 최악/평균 비율 | mixed | worst_ratio | Basic + MLFQS + CFS |
| 기아 방지: 극단적 Nice | extreme_nice | starvation_pct | Basic + MLFQS + CFS |

**메트릭 설명**:
- **cv_wait**: 변동계수 (표준편차/평균 × 100), 낮을수록 예측 가능
- **p99_wait**: 99 퍼센타일 대기 시간, 낮을수록 테일 레이턴시 양호
- **worst_ratio**: 최악/평균 대기 시간 비율, 1.0에 가까울수록 균일
- **starvation_pct**: 실행 안된 스레드 비율, 0%가 이상적

### 5. Nice 효과 (1개 테스트, MLFQS vs CFS)
...
- 절반: nice -20 (최고 우선순위)
- 절반: nice 19 (최저 우선순위)
- burst_time = 2,000 ticks (MLFQS 성능 고려하여 축소)
- **20% 시간에 중단** (일부만 완료하여 CPU 시간 비율 측정, max_ticks=20,000)

**메트릭**: CPU 시간 비율 (nice -20 그룹 / nice 19 그룹)

**관찰**: MLFQS는 매우 강한 nice 효과를, CFS는 가중치 비례 배분을 보임

### 6. 확장성 (3개 테스트, 3-way 비교)
...
- 절반: nice -20, burst_time 2,000
- 절반: nice 19, burst_time 2,000
- I/O: 없음
- 용도: Nice 효과 검증 (MLFQS 성능 고려하여 축소)

#### 9. Extreme Nice Fairness (공정성 측정용)
- 절반: nice -20, burst_time 1,000
- 절반: nice 19, burst_time 1,000
- I/O: 없음
- 용도: 제한된 시간 내 가중치 비례 공정성 측정
...
│   │   └── insights.py             # Insight 생성 및 비교 보고서
│   │
│   └── benchmark/                  # 벤치마크 정의
│       └── tests.py                # 18개 테스트 정의
│
...
**해결**:
1. `extreme_nice` 워크로드의 burst_time: 300 → 2,000 (MLFQS 성능 고려)
2. Nice 효과 테스트만 **20% 시간에 시뮬레이션 중단** (max_ticks=20,000)
...
**결과**: MLFQS는 매우 강한 nice 효과, CFS는 가중치 비례 배분
...
각 스케줄러가 자신의 강점에 맞는 워크로드에서 승리하며, 특정 스케줄러가 압도적으로 유리하지 않음.
...
**Nice 효과 측정 시**:
- burst_time 2,000 사용 (MLFQS 성능 고려)
- 20% 시간에 중단하여 부분 완료 관찰 (max_ticks=20,000)
- 이것이 올바른 방법
...
### 프로젝트 범위
3가지 실제 운영체제 스케줄러(Basic Priority, MLFQS 64-Queue, Linux CFS)를 목표 기반 테스트로 비교하는 벤치마크 시스템. 18개 테스트, 6개 카테고리로 구성되어 각 스케줄러의 강점과 약점을 정량적으로 측정.
...
프로젝트를 시작하면서 가장 큰 고민은 **"어떻게 하면 운영체제 이론을 실제로 검증할 수 있을까?"**였습니다. OS를 학습하면서 배운 CPU 스케줄링 알고리즘들은 이론적으로는 명확했지만, 실제로 어느 것이 더 좋은지, 왜 Linux가 CFS를 채택했는지, MLFQS는 어떤 상황에서 유리한지 등의 질문에 대한 명확한 답이 없었습니다.
...
#### 문제 발견과 증상
처음 벤치마크를 실행했을 때 **모든 스케줄러가 정확히 같은 결과**를 냈습니다. 평균 대기 시간, 반환 시간, 공정성 지수 모두 동일. 개선율은 모두 0%, 승자는 항상 tie.
...
#### 결과와 검증
Time slice를 추가한 후, 드디어 각 스케줄러가 서로 다른 결과를 보이기 시작했습니다. 수정 전에는 모든 스케줄러의 결과가 동일했지만, 수정 후에는 각 스케줄러가 자신만의 특성에 따라 다른 성능을 보여주었습니다.
...
CFS는 제대로 작동하는데, MLFQS만 1:1이었습니다.
...
After:  CPU time ratio에서 nice -20과 nice 19의 극단적 차이 발생
```
...
1. **burst_time 증가**: 300 → 2,000 (MLFQS 성능 고려)
2. **시뮬레이션 조기 종료**: 20% 시간에 중단 (max_ticks=20,000)
...
```
nice -20 스레드: 대부분 완료
nice 19 스레드: 거의 시작 안 함
→ CPU time ratio에서 큰 차이 발생
```

#### 결과
MLFQS는 매우 강한 nice 효과를, CFS는 가중치 비례 배분을 보임.

#### 배운 점
- **측정 방법론의 중요성**: "무엇을 측정할 것인가"만큼 "어떻게 측정할 것인가"도 중요합니다.
- **완료 vs 진행 중**: Nice 효과는 "진행 중"일 때 드러나므로, 의도적으로 중단해야 합니다.
- **이론과 실험의 차이**: 이론값과 실제값의 차이는 컨텍스트 스위치 오버헤드, time slice 크기 등 실제 환경의 영향입니다.
...
#### 문제 발견
테스트를 반복하던 중, Basic Priority가 이상하게 자주 승리했습니다. MLFQS나 CFS보다 거의 항상 빠른 결과를 보였습니다.
...
#### 결과
수정 후, Basic의 부당한 이점이 제거되었고 각 스케줄러가 자신의 **진짜 강점**에서만 승리하게 되었습니다.
...
#### 결과
수정 전에는 context_switches가 항상 0으로 나왔지만, 수정 후에는 의미 있는 측정이 가능해졌습니다. CFS가 약간 더 많은 컨텍스트 스위치를 발생시키지만, 차이는 크지 않습니다.
...
| 목표 | 잘못된 방법 | 올바른 방법 | 이유 |
|------|-----------|-----------|------|
| Nice 효과 | 모두 완료 후 CPU 시간 측정 | 20% 시간에 중단 | 완료되면 모두 burst_time만큼 사용 (1:1) |
...
**결과:**
nice -20 스레드가 거의 독점적으로 실행되고, nice 19 스레드는 거의 실행 기회를 못 받음.

---

**왜 CFS는 이론값보다 약한가?**

이론적으로 CFS는 정확히 weight 비율대로 CPU를 배분해야 합니다. 하지만 실제 결과는 이론값보다 약합니다.
...
Nice 효과 테스트는 total_work의 50%에서 중단합니다. 이 시점에서 nice -20 스레드는 대부분 완료되지만, nice 19 스레드는 거의 시작도 못 합니다. 하지만 time slice 때문에 nice 19도 최소한의 진행을 하면서, 이론값보다 약해집니다.
...
**결론:**
CFS의 결과는 "이론값보다 약하다"는 것이 아니라, **"현실 세계의 제약(time slice, 오버헤드) 속에서 달성한 강력한 nice 효과"**입니다.

---

##### 2. Basic Priority의 건투 - 단순함이 때로는 최선

세 스케줄러가 비슷한 승률을 보입니다. 처음 보는 사람은 이상하게 생각할 수 있습니다. "가장 단순한 Basic이 가장 복잡한 CFS만큼 이긴다고?"
...
**결론:**
CFS의 약간 더 많은 컨텍스트 스위치는 **"공정성의 대가로 충분히 받아들일 만한 수준"**입니다. 거의 무시할 수 있는 오버헤드입니다.

실제 Linux 시스템에서도 CFS의 컨텍스트 스위치 오버헤드는 측정하기 어려울 정도로 작으며, 그 대가로 얻는 가중치 기반 공정성은 훨씬 큰 가치가 있습니다.
...
현재 시뮬레이션에서는 세 스케줄러가 유사한 성능을 보입니다. 이는 I/O 패턴이 단순화되어 있기 때문입니다.

**참고:** 실제 Linux 환경에서 MLFQS(BSD 스타일)가 I/O-bound 작업에서 강점을 보이는 이유는 아래에 설명된 `recent_cpu` 메커니즘 때문입니다.

**MLFQS의 I/O 우대 메커니즘 (이론):**
...
#### CFS 이론값과 실제값 괴리
CFS의 실제 측정값이 이론적 weight 비율보다 약하게 나옵니다.
