...
→ 현재 시뮬레이션에서는 세 스케줄러가 유사한 성능을 보임
```

**참고:** 실제 Linux 환경에서 MLFQS(BSD 스타일)가 I/O-bound 작업에서 강점을 보이는 이유는 아래에 설명된 `recent_cpu` 메커니즘 때문입니다. 다만 현재 시뮬레이션에서는 I/O 패턴이 단순화되어 있어 이 차이가 크게 드러나지 않습니다.
